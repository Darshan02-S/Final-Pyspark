...
from pyspark.sql.functions import col, date_sub, current_date, avg
...
while True:
    ...
    def data_entry(filename):
        ...
        mycursor.execute("insert into service2(Filename, Status_of_File) values(%s, %s)", [filename, "completed"])
        conn.commit()
    ...
    def check(filename):
        ...
        mycursor = conn.cursor()
        mycursor.execute("use pyspark")
        mycursor.execute("select * from service2")
    ...
    def service():
        ...
        df_1 = df_1.na.drop()
        df_1 = df_1.filter(col("Date") < date_sub(current_date(), -14)).select("*")
        ...
        df.repartition(1).write.mode("overwrite").csv("E:\github\COVID-19\csse_covid_19_data\csse_covid_19_daily_reports\\top_three_province", header=True)
